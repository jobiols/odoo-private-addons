        ...
        if self.backup_destination == 'folder':
            ...
            try:
                ...
                if isinstance(ex, PermissionError):
                    raise UserError(_('Can not open folder %s. Permission denied') % self.folder_path)
                if isinstance(ex, FileNotFoundError):
                    raise UserError(_('Can not access %s. File or folder not found') % self.folder_path)
            ...
            if self.automatic_backup_id.delete_old_backups:
                ...
                for backup in files:
                    if re.match(backup_pattern, backup):
                        ...
                        if filedate + timedelta(days=self.automatic_backup_id.delete_days) < datetime.today():
                            os.remove(backup)
